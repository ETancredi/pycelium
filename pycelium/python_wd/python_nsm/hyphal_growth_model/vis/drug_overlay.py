    ...
    for section in mycel.get_all_segments():
        for start, end in section.get_subsegments():
            ...
            ax.plot(
                [x0, x1],
                [y0, y1],
                color=getattr(section, "color", (0.0, 0.0, 0.0)),
        ...
        if section.is_tip and not section.is_dead:
            ...
            ax.scatter(
                ...
                [y_tip],
                s=14,
                c=[getattr(section, "color", (0.0, 0.0, 0.0))],
                edgecolors="white",
                linewidths=0.6,
                zorder=4,
            )

    # Overlay terminal hyphal segments/tips that have reached zero drug-adjusted
    # growth.  This is separate from lineage colour: lineage colours still show
    # ancestry, while the zero-growth overlay marks hyphae that failed under drug.
    opts = getattr(mycel, "options", None)
    if bool(getattr(opts, "highlight_zero_growth_hyphae", True)):
        highlight_color = getattr(opts, "zero_growth_highlight_color", (1.0, 0.15, 0.0))
        highlight_width = getattr(opts, "zero_growth_highlight_linewidth", 2.6)
        marker_size = getattr(opts, "zero_growth_highlight_marker_size", 34.0)
        for section in mycel.get_all_segments():
            if getattr(section, "drug_growth_stopped_by_drug", False):
                ...
                if last_segment is not None:
                    ...
                    ax.plot([start[0], end[0]], [start[1], end[1]], color=highlight_color, linewidth=highlight_width, solid_capstyle="round", path_effects=line_effects, zorder=5)
                x_tip, y_tip = section.end.coords[:2]
