...
def load_options_from_json(path: str) -> Options:
    ...
    # Compatibility with earlier MIC-mutation test configs. These keys used to
    # make mutation probability explicitly density-dependent, but that model was
    # removed: density now increases mutation opportunity only indirectly by
    # producing more new sections/branches.
    for obsolete_key in (
